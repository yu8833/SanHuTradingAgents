...
- **综合研判**：聚合大盘 / 情绪 / 概念 / 资讯四维数据，AI 结构化输出当日方向研判（偏多 / 偏空 / 中性）+ 置信度 + 操作要点 + 今日观察 + 操作策略（含仓位建议），直接服务买卖决策
- **大盘看板**：指数行情、涨跌家数广度、涨跌停统计、情绪评分实时总览
- **短线情绪**：涨停梯队、连板高度、封板率/炸板率、晋级率等情绪指标
- **概念分析**：概念涨跌双向条形图 + 资金流入/流出双榜，概念代码直达同花顺详情
- **资讯雷达**：12 赛道 108 个 RSS 源资讯聚合，行业维度分类浏览

### 🎯 核心交易主线（候选池 → 自选 → 交易 → 分析 → 策略）

- **候选池**：行业 → 个股 → 择时 三层流水线，自动更新；每日 9:00 自动预生成当日计划快照（打开即读），行业方向预测 → 候选池 → 计划装配全链路后台异步执行
...
- **交易复盘**：买入卖出一行览 + 手动复盘笔记，沉淀「决策 → 执行 → 复盘 → 改进」经验飞轮
...
- **持仓追踪**：投资组合主页（首屏速览卡片直达），持仓管理、多币种收益跟踪与策略收益统计
- **模拟交易系统**：虚拟交易环境，验证投资策略效果
- **交易复盘**：买入卖出一行览 + 复盘笔记，沉淀「决策 → 执行 → 复盘 → 改进」经验飞轮
...
│   ├── routers/
│   │   ├── war_room.py           # 当日计划 / 盘中指导 / 当日成交聚合 API
│   │   ├── macro.py              # 宏观快扫 API

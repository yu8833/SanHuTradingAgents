...
def get_bias_threshold() -> float:
    ...
    try:
        ...
        config = get_config()
        return getattr(config, 'bias_threshold', DEFAULT_BIAS_THRESHOLD)
    except Exception:
...
class StockTrendAnalyzer:
    ...
    def analyze(self, df: pd.DataFrame, code: str) -> TrendAnalysisResult:
        ...
        result.ma30 = float(latest.get('MA30', 0))
    ...
    def _generate_signal(self, result: TrendAnalysisResult) -> None:
        ...
        if bias != bias or bias is None:  # NaN or None defense
            bias = 0.0
        base_threshold = get_bias_threshold()

    ...
    <div class="page-hero">
      <div class="page-hero-main">
        ...
        <div class="page-hero-text">
          <h2 class="page-hero-title">MACD背离策略</h2>
          <p class="page-hero-sub">
            通过MACD指标与股价走势的背离关系捕捉趋势反转信号的技术分析策略
          </p>
        </div>
      </div>
    </div>
      ...
      <el-collapse-item name="intro">
        <template #title>
          <div class="collapse-title">
            ...
            <span>策略原理</span>
            <el-tag type="warning" size="small" effect="plain">底背离/顶背离</el-tag>
          </div>
        </template>
        <div class="strategy-detail">
          <p class="strategy-overview">
            <strong>核心逻辑：</strong>当股价创出新低，但MACD指标并未同步创出新低，形成<strong>底背离</strong>，预示下跌动能衰竭，可能出现反弹；当股价创出新高，但MACD指标并未同步创出新高，形成<strong>顶背离</strong>，预示上涨动能衰竭，可能出现回调。
          </p>
          <p class="strategy-overview" style="margin-top: 12px;">
            <strong>关键信号：</strong>底背离（股价新低MACD不新低）、顶背离（股价新高MACD不新高）。背离强度越大，反转信号越可靠。
          </p>
          <p class="strategy-overview" style="margin-top: 12px;">
            <strong>风险提示：</strong>背离信号可能提前出现，需等待价格确认；大趋势下的背离可能失效，需结合均线位置和成交量综合判断。
          </p>
        </div>
      </el-collapse-item>
    </el-collapse>
      ...
      <el-tab-pane label="扫描结果" name="scan">
        <el-card class="result-panel" shadow="never">
          ...
          >
            ...
            <el-table-column prop="divergence_strength" label="背离强度" width="110" sortable align="right">
              <template #default="{ row }">
                <span :class="['pct', (row.divergence_strength ?? 0) > 0 ? 'up' : 'down']">
                  {{ fmtPctFromFraction(row.divergence_strength ?? 0, 2) }}
                </span>
              </template>
            </el-table-column>
            <el-table-column prop="macd_value" label="MACD值" width="100" sortable align="right">
              <template #default="{ row }">{{ fmtNum(row.macd_value) }}</template>
            </el-table-column>
...
const STORAGE_KEY = 'macd_divergence_scan_result'
const BACKTEST_STORAGE_KEY = 'macd_divergence_backtest_result'
...
const doScan = async () => {
  ...
  try {
    const resp = await screeningApi.scanMacdDivergence(params)
...
const doBacktest = async () => {
  ...
  try {
    ...
    const resp = await screeningApi.backtestMacdDivergence(payload)
...
const openBuyDialog = (row: any) => {
  ...
  buyTarget.strategy = 'macd_divergence'
...
const getSignalTypeTag = (type?: string) => {
  if (!type) return ''
  if (type.includes('底背离')) return 'success'
  if (type.includes('顶背离')) return 'danger'
  return 'info'
}

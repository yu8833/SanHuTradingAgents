  ...
  <div class="ma-crossover">
    <div class="page-header">
      ...
      <p class="page-description">
        通过移动平均线金叉、死叉信号捕捉趋势转折点的经典技术分析策略
      </p>
    </div>
      ...
      <el-collapse-item name="intro">
        <template #title>
          <div class="collapse-title">
            ...
            <span>策略原理</span>
            <el-tag type="success" size="small" effect="plain">金叉/死叉/均线排列</el-tag>
          </div>
        </template>
      ...
      <el-tab-pane label="扫描结果" name="scan">
        <el-card class="result-panel" shadow="never">
          ...
          >
            ...
            <el-table-column prop="ma5" label="MA5" width="90" sortable>
              <template #default="{ row }">{{ formatNum(row.ma5) }}</template>
            </el-table-column>
            <el-table-column prop="ma10" label="MA10" width="90" sortable>
              <template #default="{ row }">{{ formatNum(row.ma10) }}</template>
            </el-table-column>
            <el-table-column prop="ma20" label="MA20" width="90" sortable>
...
const STORAGE_KEY = 'ma_crossover_scan_result'
const BACKTEST_STORAGE_KEY = 'ma_crossover_backtest_result'
...
const doScan = async () => {
  ...
  tookMs.value = 0
  try {
    const resp = await screeningApi.scanMaCrossover(params)
...
const doBacktest = async () => {
  ...
  try {
    ...
    const resp = await screeningApi.backtestMaCrossover(payload)
...
const openBuyDialog = (row: any) => {
  ...
  buyTarget.strategy = 'ma_crossover'
...
const getSignalTypeTag = (type?: string) => {
  if (!type) return ''
  if (type.includes('金叉') || type.includes('多头')) return 'success'
  if (type.includes('死叉') || type.includes('空头')) return 'danger'
  return 'info'
}

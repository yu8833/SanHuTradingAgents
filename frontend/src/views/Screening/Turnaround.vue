    ...
    <div class="page-hero">
      <div class="page-hero-main">
        ...
        <div class="page-hero-text">
          <h2 class="page-hero-title">困境反转</h2>
          <p class="page-hero-sub">
            寻找最差时候已过去的公司，在底部布局等待业绩验证的困境反转策略
          </p>
        </div>
      </div>
    </div>
      ...
      <el-collapse-item name="intro">
        <template #title>
          <div class="collapse-title">
            ...
            <span>策略原理</span>
            <el-tag type="warning" size="small" effect="plain">底部布局/业绩反转</el-tag>
          </div>
        </template>
        <div class="strategy-detail">
          <p class="strategy-overview">
            <strong>核心逻辑：</strong>当一家公司经营处于历史最差时期（亏损、行业谷底、利空频发），
            但行业供需出现拐点、公司开始自救（管理层变更、资产剥离、降本增效），
            股价已反映悲观预期，此时逢低布局等待业绩验证，可获得戴维斯双击收益。
          </p>
          <p class="strategy-overview" style="margin-top: 12px;">
            <strong>关键信号：</strong>距历史低点涨幅有限（&lt;30%）、近20日有反弹迹象、成交量温和放大、
            估值水平合理（PE分位30%-60%）、行业出现改善迹象、信号类型分为左侧潜伏与右侧确认两种。
          </p>
          <p class="strategy-overview" style="margin-top: 12px;">
            <strong>风险提示：</strong>困境反转耗时不确定，可能持续6-12个月，需有耐心；
            分批建仓摊薄成本；若业绩持续恶化需果断止损，避免"价值陷阱"。
          </p>
        </div>
      </el-collapse-item>
    </el-collapse>
      ...
      <el-tab-pane label="扫描结果" name="scan">
        <el-card class="result-panel" shadow="never">
          ...
          >
            ...
            <el-table-column prop="rise_from_low" label="距低点涨幅" width="120" sortable align="right">
              <template #default="{ row }">
                <span :class="['pct', (row.rise_from_low ?? 0) >= 0 ? 'up' : 'down']">
                  {{ (row.rise_from_low ?? 0) >= 0 ? '+' : '' }}{{ (row.rise_from_low ?? 0).toFixed(2) }}%
                </span>
              </template>
            </el-table-column>
            <el-table-column prop="recent_20d_chg" label="近20日涨幅" width="120" sortable align="right">
              <template #default="{ row }">
                <span :class="['pct', (row.recent_20d_chg ?? 0) >= 0 ? 'up' : 'down']">
                  {{ (row.recent_20d_chg ?? 0) >= 0 ? '+' : '' }}{{ (row.recent_20d_chg ?? 0).toFixed(2) }}%
                </span>
              </template>
            </el-table-column>
            <el-table-column prop="volume_change" label="成交量变化" width="120" sortable align="right">
              <template #default="{ row }">
                <span :class="['pct', (row.volume_change ?? 0) >= 0 ? 'up' : 'down']">
                  {{ (row.volume_change ?? 0) >= 0 ? '+' : '' }}{{ (row.volume_change ?? 0).toFixed(2) }}%
                </span>
              </template>
            </el-table-column>
            <el-table-column prop="valuation_level" label="估值水平" width="110">
              <template #default="{ row }">
                <el-tag :type="getValuationTag(row.valuation_level)" size="small">
                  {{ row.valuation_level || '-' }}
                </el-tag>
              </template>
            </el-table-column>
...
const STORAGE_KEY = 'turnaround_scan_result'
const BACKTEST_STORAGE_KEY = 'turnaround_backtest_result'
...
const getValuationTag = (level?: string) => {
  if (!level) return ''
  if (level.includes('低估') || level.includes('低')) return 'success'
  if (level.includes('合理')) return 'info'
  if (level.includes('高估') || level.includes('高')) return 'danger'
  return ''
}

  ...
  <div class="volume-price">
    <div class="page-header">
      ...
      <p class="page-description">
        通过成交量与价格的配合关系判断市场强弱和趋势持续性的经典技术分析策略
      </p>
    </div>
      ...
      <el-collapse-item name="intro">
        <template #title>
          <div class="collapse-title">
            ...
            <span>策略原理</span>
            <el-tag type="primary" size="small" effect="plain">放量上涨/缩量回调</el-tag>
          </div>
        </template>
        <div class="strategy-detail">
          <p class="strategy-overview">
            <strong>核心逻辑：</strong>"量在价先"是技术分析的基本原则。当价格上涨时成交量同步放大，说明上涨有资金推动，是<strong>放量上涨</strong>的健康形态；当价格回调时成交量萎缩，说明抛压减轻，是<strong>缩量回调</strong>的良性调整。
          </p>
          <p class="strategy-overview" style="margin-top: 12px;">
            <strong>关键信号：</strong>放量上涨、缩量回调、量价齐升。量能倍数和量价相关系数是判断量价配合程度的重要指标。
          </p>
          <p class="strategy-overview" style="margin-top: 12px;">
            <strong>风险提示：</strong>高位放量可能是出货信号，低位缩量可能是交投低迷；需结合价格形态和相对位置综合判断。
          </p>
        </div>
      </el-collapse-item>
    </el-collapse>
      ...
      <el-tab-pane label="扫描结果" name="scan">
        <el-card class="result-panel" shadow="never">
          ...
          >
            ...
            <el-table-column prop="volume_multiple" label="量能倍数" width="110" sortable>
              <template #default="{ row }">
                <span :class="['pct', (row.volume_multiple ?? 0) > 1 ? 'up' : 'down']">
                  {{ (row.volume_multiple ?? 0).toFixed(2) }}倍
                </span>
              </template>
            </el-table-column>
            <el-table-column prop="price_volume_correlation" label="量价相关系数" width="130" sortable>
              <template #default="{ row }">{{ formatNum(row.price_volume_correlation) }}</template>
            </el-table-column>
...
const STORAGE_KEY = 'volume_price_scan_result'
const BACKTEST_STORAGE_KEY = 'volume_price_backtest_result'
...
const doScan = async () => {
  ...
  tookMs.value = 0
  try {
    const resp = await screeningApi.scanVolumePrice(params)
...
const doBacktest = async () => {
  ...
  try {
    ...
    const resp = await screeningApi.backtestVolumePrice(payload)
...
const openBuyDialog = (row: any) => {
  ...
  buyTarget.strategy = 'volume_price'
...
const getSignalTypeTag = (type?: string) => {
  if (!type) return ''
  if (type.includes('放量上涨') || type.includes('量价齐升')) return 'success'
  if (type.includes('缩量回调')) return 'warning'
  if (type.includes('温和上涨')) return 'info'
  return ''
}

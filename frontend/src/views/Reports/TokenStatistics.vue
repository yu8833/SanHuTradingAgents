...
const stats = ref<UsageStatistics>({
  total_requests: 0,
...
const loadStatistics = async () => {
  ...
  try {
    ...
    if (res.success) {
      ...
      overview.totalRequests = res.data.total_requests
      overview.totalTokens = res.data.total_input_tokens + res.data.total_output_tokens
      overview.totalCost = res.data.total_cost
      overview.avgCostPerRequest = res.data.total_requests > 0 
        ? res.data.total_cost / res.data.total_requests 
        : 0
      ...
      modelRanking.value = Object.entries(res.data.by_model)
        .map(([name, data]) => ({
...
const loadRecords = async () => {
  try {
    ...
    if (res.success) {
      records.value = res.data.records.map((r: UsageRecord) => ({
...
const renderCharts = () => {
  // Token使用趋势图
  if (tokenTrendChart.value && stats.value.by_date) {
    const chart1 = echarts.init(tokenTrendChart.value)
    
    const sortedDates = Object.keys(stats.value.by_date).sort()
    const tokenData = sortedDates.map(date => 
      stats.value.by_date[date].input_tokens + stats.value.by_date[date].output_tokens
    )
  ...
  // 成本分布图
  if (costDistributionChart.value && stats.value.by_provider) {
    const chart2 = echarts.init(costDistributionChart.value)
    
    const pieData = Object.entries(stats.value.by_provider).map(([name, value]) => ({
  ...
  // 供应商统计图
  if (providerChart.value && stats.value.by_provider) {
    const chart3 = echarts.init(providerChart.value)
    
    const providerData = Object.entries(stats.value.by_provider).map(([name, value]) => ({
      name: getProviderName(name),
    ...
    chart3.setOption({
      tooltip: { trigger: 'axis' },
      xAxis: { type: 'category', data: providerData.map(p => p.name) },
      yAxis: { type: 'value' },
      series: [{
        data: providerData.map(p => p.requests),
        type: 'bar'
      }]
    })
  }

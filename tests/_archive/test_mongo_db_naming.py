...
class MongoDbNamingTests(unittest.TestCase):
    def test_auto_scope_debug_uses_major_instance(self):
        ...
        try:
            ...
            s = Settings()
            assert s.MONGO_DB == "tradingagentscn_v1_devx"
            identity = s.MONGO_DB_IDENTITY
            assert identity["scope_effective"] == "major_instance"
    ...
    def test_auto_scope_prod_uses_explicit(self):
        ...
        try:
            os.environ.update(env)
            s = Settings()
            assert s.MONGO_DB == "tradingagentscn"
            identity = s.MONGO_DB_IDENTITY
            assert identity["scope_effective"] == "explicit"
        finally:
            for k, v in old.items():
                if v is None:
                    os.environ.pop(k, None)
    ...
    def test_debug_shared_db_requires_explicit_override(self):
        ...
        try:
            os.environ.update(env)
            s = Settings()
            assert s.MONGO_DB_IDENTITY["scope_effective"] == "explicit"
            assert not s.ALLOW_SHARED_DB_IN_DEBUG
        finally:
            for k, v in old.items():
                if v is None:
                    os.environ.pop(k, None)

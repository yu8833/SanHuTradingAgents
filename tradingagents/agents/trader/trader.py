...
- **入场价**：XXX 元（必须是纯数字价格，如 28.4 元；若不建议入场写"不追高"）
- **第一目标位**：XXX 元（纯数字价格，如 29.5 元；买入/持有为上方目标位，卖出/减持为下行退出位，方向须与评级一致）
- **第二目标位**：XXX 元（可选，纯数字价格；方向同上）
- **止损位**：XXX 元（纯数字价格，如 26.1 元；此项必须给出。买入/持有为入场价下方；卖出/减持为空头失效位，位于价格上方，勿与支撑位混淆）
- **支撑位**：XXX 元（纯数字价格，与止损位可以不同；此项必须给出）
- **风险/回报比**：直接输出比值，如"约1:2.0"，仅一个比值，不要附加说明句子
- **建议仓位**：仓位比例和建仓方式
